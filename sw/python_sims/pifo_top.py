...
class Pifo_top(HW_sim_object):
    ...
    def __init__(self, env, period, pkt_in_pipe, pkt_out_pipe, enq_out_pipe, deq_in_pipe, max_segments, max_pkts, num_skip_lists, outreg_width, enq_fifo_depth, rd_latency=1, wr_latency=1, sl_impl='det'):
        ...
        self.skip_list_wrapper = SkipListWrapper(env, self.sl_enq_in_pipe, self.sl_enq_out_pipe, self.top_deq_in_pipe, self.sl_deq_out_pipe, num_sl=num_skip_lists, period=period, size=max_pkts, outreg_width=outreg_width, enq_fifo_depth=enq_fifo_depth, rd_latency=rd_latency, wr_latency=wr_latency, sl_impl=sl_impl)
